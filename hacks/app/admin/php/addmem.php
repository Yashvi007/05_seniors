...
if (isset($_POST['upload'])) {
    ...
    $q1="SELECT count(*) as count FROM faculty";
...
header("Location: ../viewdep.php?id=" . $depid. "&name=" . $depname);
    ...
    if (!isset($_SESSION['error'])) {
        ...
        $sql = "INSERT INTO faculty(eid,fname,lname,email,gender,depid,designation,phone,password) VALUES(?,?,?,?,?,?,?,?,?);";
        $stmt = mysqli_stmt_init($connect);
        if (!mysqli_stmt_prepare($stmt, $sql)) {
            echo "Sql Statement failed";
        } else {
            mysqli_stmt_bind_param($stmt, "sssssssss",$eid, $fname, $lname,$encryption,$gender,$depid,$designation,$encryption1,$hash);
